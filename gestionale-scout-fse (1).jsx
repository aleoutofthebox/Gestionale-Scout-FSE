...
// Versione con archivio persistente condiviso (window.storage)
// - I dati si salvano automaticamente e restano alla chiusura
// - Più persone possono lavorare sugli stessi dati
// ============================================================
...
// Chiavi: "config" (impostazioni), "mov.<id>" (movimenti), "socio.<id>" (soci)
...
const modalitaArchivio = () =>
  condivisoDisponibile() ? "condiviso" : localeDisponibile() ? "locale" : "memoria";
...
const Archivio = {
  _backend() {
    return modalitaArchivio() === "condiviso" ? ArchivioCondiviso : ArchivioLocale;
  },
...
function reducer(state, action) {
  switch (action.type) {
    ...
    case "AGGIUNGI_MOVIMENTO":
      return { ...state, movimenti: [...state.movimenti, action.payload] };
    case "ELIMINA_MOVIMENTO":
      return { ...state, movimenti: state.movimenti.filter((m) => m.id !== action.id) };
    case "AGGIUNGI_SOCIO":
      return { ...state, soci: [...state.soci, action.payload] };
    case "ELIMINA_SOCIO":
      return { ...state, soci: state.soci.filter((s) => s.id !== action.id) };
    case "IMPOSTA_CONFIG":
...
function useContabilita() {
  ...
  const ricarica = useCallback(async (silenzioso = false) => {
    ...
    try {
      const [config, movimenti, soci] = await Promise.all([
        Archivio.leggi("config"),
        Archivio.leggiTutti("mov."),
        Archivio.leggiTutti("socio."),
      ]);
      dispatch({
        type: "CARICATO",
  ...
  useEffect(() => {
    ricarica();
    if (modalitaArchivio() !== "condiviso") return;
    const intervallo = setInterval(() => ricarica(true), 25000);
    return () => clearInterval(intervallo);
  }, [ricarica]);

  const conArchivio = useCallback(
    async (operazione, azioneLocale) => {
      if (state.modalita === "memoria") {
        // Nessun archivio: applica comunque localmente
        azioneLocale();
        return true;
      }
      try {
  ...
  const azioni = useMemo(
    () => ({
      ...
      aggiungiMovimento: async (dati) => {
        const mov = { ...dati, id: genId(), creatoIl: Date.now() };
        return conArchivio(
          () => Archivio.scrivi(`mov.${mov.id}`, mov),
          () => dispatch({ type: "AGGIUNGI_MOVIMENTO", payload: mov })
        );
      },

      eliminaMovimento: async (id) =>
        conArchivio(
          () => Archivio.elimina(`mov.${id}`),
          () => dispatch({ type: "ELIMINA_MOVIMENTO", id })
        ),

      aggiungiSocio: async (dati) => {
        const socio = { ...dati, id: genId() };
        return conArchivio(
          () => Archivio.scrivi(`socio.${socio.id}`, socio),
          () => dispatch({ type: "AGGIUNGI_SOCIO", payload: socio })
        );
      },

      eliminaSocio: async (id) =>
        conArchivio(
          () => Archivio.elimina(`socio.${id}`),
          () => dispatch({ type: "ELIMINA_SOCIO", id })
        ),

      salvaConfig: async (parziale) => {
        const nuova = { ...configCorrente(), ...parziale };
        return conArchivio(
          () => Archivio.scrivi("config", nuova),
      ...
      importaBackup: async (dati) => {
        ...
        if (modalitaArchivio() === "memoria") {
          dispatch({ type: "CARICATO", payload: { modalita: "memoria", ...payloadDaConfig(dati.config), movimenti, soci } });
          return true;
        }
        try {
          const [vecchiMov, vecchiSoci] = await Promise.all([Archivio.elenco("mov."), Archivio.elenco("socio.")]);
          await Promise.all([...vecchiMov, ...vecchiSoci].map((k) => Archivio.elimina(k)));
          if (dati.config) await Archivio.scrivi("config", dati.config);
          await Promise.all([
            ...movimenti.map((m) => Archivio.scrivi(`mov.${m.id}`, m)),
            ...soci.map((s) => Archivio.scrivi(`socio.${s.id}`, s)),
          ]);
          await ricarica();
          return true;
        } catch (e) {
    ...
    [conArchivio, configCorrente, ricarica]
  );
...
function FormMovimento({ categorie, onAggiungi }) {
  ...
  const salva = async () => {
    ...
    if (importo <= 0) return setErrore("Inserisci un importo maggiore di zero.");
    setErrore("");
    setInCorso(true);
    ...
    <div className="rounded-lg border border-stone-200 bg-white p-5">
      ...
      {!aperto ? (
        ...
        <div className="mt-3 grid grid-cols-2 gap-3">
          ...
          <button onClick={conferma} className="col-span-2 rounded-md bg-[#1b2a4a] px-4 py-2 text-sm text-white">
            Salva saldi iniziali
          </button>
        </div>
      )}
    </div>
              ...
              <td className="px-3 py-2">
                ...
                {(m.attivita || (m.socioId && nomeSocio(m.socioId))) && (
                  <div className="text-xs text-stone-400">
                    {[m.attivita, m.socioId ? (nomeSocio(m.socioId) ? `Socio: ${nomeSocio(m.socioId)}` : null) : null].filter(Boolean).join(" · ")}
                  </div>
                )}
              </td>
...
function RegistraQuota({ socio, quotaAnnuale, onConferma, onAnnulla }) {
  ...
  const [importo, setImporto] = useState(String(quotaAnnuale).replace(".", ","));
    ...
    <tr className="border-b border-stone-100 bg-blue-50/50">
      <td colSpan={6} className="px-3 py-3">
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div className="w-28">
            <label className={stileLabel}>Importo</label>
            <input type="text" inputMode="decimal" value={importo} onChange={(e) => setImporto(e.target.value)} className={stileCampo} />
          </div>
          <div>
      ...
      <div className="space-y-6 lg:col-span-2">
        ...
        <div className="rounded-lg border border-stone-200 bg-white p-5">
          ...
          <div className="mt-3 flex gap-2">
            ...
            <button
              onClick={() => azioni.salvaConfig({ quotaAnnuale: parseImporto(nuovaQuota) || quotaAnnuale })}
...
function GestioneCategorie({ categorie, movimenti, azioni }) {
  const [nuova, setNuova] = useState({ entrata: "", uscita: "" });

  const inUso = (nome) => movimenti.some((m) => m.categoria === nome);
  const predefinita = (tipo, nome) => CATEGORIE_BASE[tipo].includes(nome);

  const aggiungi = (tipo) => {
    const nome = nuova[tipo].trim();
    if (!nome || categorie[tipo].includes(nome)) return;
    azioni.salvaConfig({ categorie: { ...categorie, [tipo]: [...categorie[tipo], nome] } });
    setNuova({ ...nuova, [tipo]: "" });
  };

  const rimuovi = (tipo, nome) => {
    azioni.salvaConfig({ categorie: { ...categorie, [tipo]: categorie[tipo].filter((c) => c !== nome) } });
  };

  const Colonna = ({ tipo, titolo, accento }) => (
    <div className="rounded-lg border border-stone-200 bg-white p-5">
      ...
      <ul className="mt-3 divide-y divide-stone-100">
        {categorie[tipo].map((c) => (
          <li key={c} className="flex items-center justify-between py-2 text-sm">
            <span>
              {c}
              {predefinita(tipo, c) && <span className="ml-2 text-xs text-stone-400">FSE</span>}
            </span>
            {!predefinita(tipo, c) && (
              inUso(c) ? (
                <span className="text-xs text-stone-400" title="Categoria utilizzata in prima nota">in uso</span>
              ) : (
                <button onClick={() => rimuovi(tipo, c)}
      ...
      <div className="mt-3 flex gap-2">
        <input type="text" placeholder="Nuova categoria…" value={nuova[tipo]}
          onChange={(e) => setNuova({ ...nuova, [tipo]: e.target.value })} className={stileCampo} />
        <button onClick={() => aggiungi(tipo)} className="shrink-0 rounded-md bg-[#1b2a4a] px-4 py-2 text-sm text-white">
          Aggiungi
        </button>
      </div>
    </div>
  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
      <Colonna tipo="entrata" titolo="Categorie di entrata" accento="text-emerald-800" />
      <Colonna tipo="uscita" titolo="Categorie di uscita" accento="text-red-900" />
...
function FormIntestazione({ intestazione, azioni }) {
  ...
  useEffect(() => { setForm(intestazione); }, [intestazione]);
  ...
  return (
    <div className="rounded-lg border border-stone-200 bg-white p-5">
      ...
      <div className="mt-3 space-y-3">
        ...
        <button
          onClick={() => azioni.salvaConfig({
            intestazione: { ...form, esercizio: parseInt(form.esercizio) || intestazione.esercizio },
          })}
          className="w-full rounded-md bg-[#1b2a4a] px-4 py-2 text-sm text-white">
          Salva intestazione
        </button>
      </div>
    </div>
...
function PannelloBackup({ state, azioni }) {
  ...
  const [esito, setEsito] = useState(null); // {tipo:'ok'|'errore', testo}
  const [inCorso, setInCorso] = useState(false);
  ...
  const importa = (e) => {
    ...
    reader.onload = async () => {
      try {
        ...
        setInCorso(false);
        setEsito(ok
          ? { tipo: "ok", testo: "Backup importato correttamente." }
          : null);
      } catch {
